...
Te     = 63	# Fixed entanglement time (parameter)
...
for f in fst_in:
	...
	for w in wp_tpa:
	    ...
	    if w in wp:
		mol.calc_etpa_cs(nstate,w,ist,fst,erange,k,Te,Ae,tstep,line,pol,tau)
		mol.write_etpa_cs(outfilename+'_f'+str(f)+'_wp'+'{0:.2f}'.format(w)+'_kap'+'{0:.2f}'.format(k)+'_tau'+str(Te))
